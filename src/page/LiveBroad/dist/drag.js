...
(function(){
	...
	b.prototype = {
		init:function(){
			...
			console.log(t.t.offset().top,$(window).scrollTop())
		...
		move:function(){
			...
			t.c.on('mousedown',function(e){
				...
				$(document).on('mouseup',function(e){
					t.top = t.t.offset().top - $(window).scrollTop();
					t.left = t.t.offset().left - $(window).scrollLeft();
					localStorage.videotop=t.top;
					localStorage.videoleft=t.left;
					console.log(localStorage.videotop,localStorage.videoleft)
					if(t.top == 66 && t.leave(e.pageX,e.pageY)){
						// t.top_animate();
		...
		size:function(){
			...
			t.sz.css({'position':'absolute','right':"2px",'bottom':"2px",'width':t.size_bg+'px','height':t.size_bg+'px',"border-right": "2px solid rgba(7, 253, 37, 0.65)","border-bottom": "2px solid rgba(7, 253, 37, 0.65)",'cursor':'nw-resize'});
			t.sz.on('mousedown',function(e){
				...
				$(document).on('mouseup',function(){
					t.height = t.t.outerHeight();
					t.width = t.t.outerWidth();
					localStorage.videowidth=t.width;
					localStorage.videoheight=t.height;
					t.s = false;
					$(document).off('mousemove');
					$(document).off('mouseup');
				});
		...
		top_animate:function(){
			...
			t.t.animate({top:(-t.height+t.hide)},300,function(){
				t.t.one('mouseenter',function(){
					t.t.stop(true).animate({top:70},300);
				});
			});
		},
